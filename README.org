...
**** Aspect methods
- =boolean shouldApplyEffectTickThisTick(int duration, int amplifier)=
  Determine if effect should be applied or not
- =boolean applyEffectTick(ServerLevel world, LivingEntity entity, int amplifier)=
  The main logic of the effect
...
Then build recipe for it =registerModPotions()= by specifying base,
