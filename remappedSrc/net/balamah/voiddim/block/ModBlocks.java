...
public class ModBlocks {
	protected static AbstractBlock.Settings corpseBlockSettings = AbstractBlock.Settings.create()
			.strength(1.2f)
			.sounds(BlockSoundGroup.BONE)
			.nonOpaque();

	protected static AbstractBlock.Settings flowerBlockSettings = AbstractBlock.Settings.create()
	...
	public static final Block VOIDIUM_ORE =
		register(
			"voidium_ore",
			settings -> new ExperienceDroppingBlock(UniformIntProvider.create(2, 3), settings),
	...
	public static final Block CORRUPTED_TORCH =
		register(
			...
			AbstractBlock.Settings.create().noCollision().instabreak()
			.lightLevel(state -> 14).sound(BlockSoundGroup.WOOD)
			.pushReaction(PistonBehavior.DESTROY),
			false, false
		);
	...
	public static final Block CORRUPTED_FIRE =
		register(
			...
			.lightLevel(state -> 15).sound(BlockSoundGroup.WOOL)
			.pushReaction(PistonBehavior.DESTROY)
			.noOcclusion(),
			false, false
			);
	...
	protected static AbstractBlock.Settings copyLootTable(
		...
		AbstractBlock.Settings settings2 = AbstractBlock.Settings.create()
			.overrideLootTable(block.getLootTableKey());

		if (copyTranslationKey) {
			settings2 = settings2.overrideDescription(block.getTranslationKey());
		}
	...
	protected static RegistryKey<Block> keyOfBlock(String name) {
		return RegistryKey.of(
			RegistryKeys.BLOCK, ResourceLocation.fromNamespaceAndPath(VoidDimension.MOD_ID, name)
		);
	}
	
	protected static RegistryKey<Item> keyOfItem(String name) {
		return RegistryKey.of(
			RegistryKeys.ITEM, ResourceLocation.fromNamespaceAndPath(VoidDimension.MOD_ID, name)
		);
	}
}
