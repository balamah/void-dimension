...
public class ModItems {
	...
	protected static Item.Settings prayerItemSettings() {
	...
	public static final Item CORRUPTED_FIRE_CHARGE =
		register("corrupted_fire_charge", CorruptedFireChargeItem::new, Item.Settings::new);
	...
	protected static Item register(
		...
		Supplier<Item.Settings> settingsFactory
	) {
		RegistryKey<Item> itemKey = RegistryKey.of(
			RegistryKeys.ITEM, ResourceLocation.fromNamespaceAndPath(MOD_ID, name)
		);

		Item.Settings settings = settingsFactory.get().setId(itemKey);
	...
	protected static Item registerBlockItem(
		Block block,
		BiFunction<Block, Item.Settings, Item> factory,
		Supplier<Item.Settings> settingsFactory
	) {
		RegistryKey<Block> blockKey =
			Registries.BLOCK.getKey(block).orElseThrow();
	...
	protected static Item registerByKey(
		...
		Item.Settings settings = settingsFactory.get().setId(key);
	...
	protected static RegistryKey<Item> keyOfBlock(RegistryKey<Block> blockKey) {
		return RegistryKey.of(RegistryKeys.ITEM, blockKey.identifier());
	}
	...
	protected static Item registerArmor(
		...
		return register(
			name,
			Item::new,
			() -> (isVoid ? getVoidItemSettings() : new Item.Settings())
	...
	protected static Item registerFoodItem(
		String name, FoodComponent foodComponent, Consumable consumableComponent
	) {
		return register(
			name,
			Item::new,
			() -> new Item.Settings().food(foodComponent, consumableComponent)
		);
	}
